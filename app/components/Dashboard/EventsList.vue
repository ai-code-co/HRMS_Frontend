...
<template>
    <section class="bg-white border border-slate-100 rounded-[2.5rem] overflow-hidden shadow-sm h-full">
        <div class="px-8 py-6 border-b border-slate-50 flex items-center justify-between">
            <h3 class="font-black text-slate-700 uppercase text-xs tracking-widest">
                {{ title }}
            </h3>
            <UButton variant="link" color="indigo" size="xs" :padded="false">See All</UButton>
        </div>
        <div class="divide-y divide-slate-50">
            <div v-for="(item, i) in items" :key="i"
                ...
                <UButton size="2xs" color="white" variant="solid"
                    class="font-bold uppercase tracking-widest shadow-none bg-slate-100 text-slate-600 hover:bg-indigo-600 hover:text-white transition-all">
                    {{ actionLabel }}
                </UButton>
            </div>
        </div>
