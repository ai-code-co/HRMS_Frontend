...
<template>
  <div class="space-y-8">
    ...
    <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
       <div v-for="n in 6" :key="n" class="bg-white h-40 rounded-3xl p-6 shadow-sm animate-pulse border border-slate-100"></div>
    </div>
      ...
      >
        <div class="space-y-3">
           ...
           <div class="space-y-1.5">
              ...
              <div class="flex items-center gap-3">
                 <span class="text-[10px] font-bold text-slate-300 uppercase tracking-widest min-w-[80px]">Unassigned:</span>
        ...
        <StatusChart 
          :percentage="cat.total > 0 ? Math.round((cat.working / cat.total) * 100) : 0" 
          :total="cat.working" 
        />
      </div>
    </div>
