    ...
    <div class="w-full flex bg-[#F8FAFC] font-sans text-slate-900 min-w-0">
        <div class="flex-1 flex flex-col relative min-w-0">
            ...
            <div v-if="view === 'list'" class="px-4 md:px-6 max-w-[1600px] mx-auto w-full">
                <div class="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-4 border-b border-slate-100 bg-white/50 backdrop-blur-sm rounded-xl px-6 py-4 sticky top-0 z-20 min-w-0">
                    <div>
                    ...
                    <div class="flex flex-col lg:flex-row lg:flex-wrap xl:flex-nowrap items-stretch lg:items-center gap-3 bg-white border border-slate-200 px-4 py-2 rounded-xl shadow-sm w-full lg:w-auto min-w-0">
                        <div class="flex items-center gap-1 bg-slate-100/60 p-1 rounded-xl border border-slate-200 shadow-sm w-full lg:w-auto">
                            <UButton v-for="f in ['Assigned', 'Unassigned']" :key="f" size="xs" variant="ghost" :class="[
                                'px-2 lg:px-6 py-1.5 rounded-lg text-[9px] lg:text-[10px] font-black uppercase transition-all duration-200 flex-1 lg:flex-none min-w-0 justify-center text-center',
                                activeFilter === f
                        ...
                        <USelect
                            ...
                            size="lg"
                            class="w-full lg:w-[180px] min-w-0 border border-slate-200 rounded-xl shadow-sm"
                        />
                        <USelectMenu
                        ...
                        <UButton 
                            ...
                            color="primary" 
                            size="lg"
                            class="w-full lg:w-auto border border-slate-200 rounded-xl shadow-sm justify-center text-center"
            ...
            <main class="flex-1 p-4 md:p-8 max-w-[1600px] mx-auto w-full min-w-0">
                ...
                <div v-else class="flex flex-col lg:flex-row h-full animate-fadeIn gap-6">
                    ...
                    <template v-else>
                        <div class="hidden lg:block w-full lg:w-80 h-64 lg:h-full shrink-0 overflow-auto custom-scrollbar">
                            <SidebarList :items="inventoryItems" :selected-id="selectedItemId" :loading="loadingDevices"
                                @select="handleSelectItem" />
                        </div>

                        <div class="flex-1 min-w-0 overflow-y-auto custom-scrollbar pr-0 lg:pr-2 space-y-6">
