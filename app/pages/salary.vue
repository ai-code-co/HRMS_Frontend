<template>
    <div class="flex flex-col h-full bg-[#F8FAFC]">
        <!-- Admin Organization View Header -->
        <SalaryAdminHeader v-if="isSuperUser && !selectedEmployeeId" />

        <!-- Employee Context Header (Admin viewing specific employee) -->
        <SalaryEmployeeContextHeader v-else-if="isSuperUser && selectedEmployeeId && activeEmployee"
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar">
            ...
            <SalaryAdminOrgView v-if="isSuperUser && !selectedEmployeeId" />
...
const { selectedEmployeeId, activeEmployee } = useEmployeeContext()
...
// Fetch org data for admin view
if (isSuperUser.value) {
    const { data: orgData } = await useAsyncData('salary-org', () => {
        return store.fetchOrgPayrollData(false)
    })
...
// Watch for employee changes to update salary structure
watch(selectedEmployeeId, (newId) => {
    store.setSalaryStructureForEmployee(newId)
}, { immediate: true })
